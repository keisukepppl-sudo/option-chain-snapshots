...
from morita_cloud.logic import checkpoint_candidates, eligible, normalize_et
...
def checkpoint_message(slot: str, candidates: pd.DataFrame, cutoff_et: pd.Timestamp) -> str:
    ...
    lines = [
        f"{slot} ET S+A Scan",
        f"decision_cutoff_et={normalize_et(cutoff_et).isoformat()}",
    ]
    if visible.empty:
    ...
    if slot == "12:00":
        lines.append("This set becomes the noon execution baseline.")
    return "\n".join(lines)
...
def wake_message(candidates: pd.DataFrame, state: dict[str, Any], cutoff_et: pd.Timestamp) -> str:
    ...
    if not bool(state.get("noon_snapshot_complete")):
        lines.append("WARNING: 12:00 baseline missing; fail-safe woke for every current S.")
...
def full_discord_message(title: str, candidates: pd.DataFrame, cutoff_et: pd.Timestamp) -> str:
    lines = [title, f"decision_cutoff_et={normalize_et(cutoff_et).isoformat()}"]
